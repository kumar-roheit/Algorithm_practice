class Solution:
    def productExceptSelf(self, num: List[int]) -> List[int]:
        # The Length of the number array
        length = len(num)
        
        # Initializing the left , right and the output with list of zeros
        L,R,output = [0]*length,[0]*length,[0]*length
        
        # L[i] contains the product of all the elements to the left
        # Note: for the element at index '0', there are no elements to the               left, so the L[0] would be 1
        L[0]=1
        R[length -1]=1
        
        # L[i - 1] already contains the product of elements to the left of 'i -           1' Simply multiplying it with nums[i - 1] would give the product of             all elements to the left of index 'i'
        
        for i in range(1, length):
            L[i]=L[i-1]*num[i-1]
        # print(L)
        
        # R[i] contains the product of all the elements to the right 
        # Note: for the element at index 'length - 1', there are no elements to           the right so the R[length - 1] would be 1
        
        for i in range(length-2,-1,-1):
            R[i] = num[i+1] * R[i+1] 
        # print(R)
        
        # Output
        # For the first element, R[i] would be product except self
        # For the last element of the array, product except self would be L[i]
        # Else, multiple product of all elements to the left and to the right
        
        for i in range(length):
            output[i] = L[i]*R[i]
        return output
